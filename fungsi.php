...
$datap=mysqli_query($koneksi,"SELECT * FROM pasien");
...
function tambah2()
{
	...
	$d= htmlspecialchars($_POST['d']);
   global $koneksi;
	$tambah= " INSERT INTO `gizi` VALUES('','$b', '$c', '$d');";
	$cek=$koneksi->query($tambah);
	return mysqli_affected_rows($koneksi);
}
function hapus($b)
{
	global $koneksi;
		mysqli_query($koneksi,"DELETE FROM pasien WHERE id='$b'");
		return mysqli_affected_rows($koneksi);
}
function hapus1($c)
{
...
global $koneksi;
	...
	$ubah="UPDATE `pasien` SET  id='$k' ,norm='$a', noidpasien='$b', jenisid='$c', namapasien='$d', alamat='$e', agama='$f', golongandarah='$g', tanggallahir='$h', jeniskelamin='$i', waktudaftar='$j' WHERE id='$k'";
...
global $koneksi;
	...
	$c= htmlspecialchars($_POST['c']);
	$d= htmlspecialchars($_POST['d']);
$ubah1="UPDATE `ruangan` SET noruangan='$a', norm='$b', waktumasuk='$c', waktukeluar='$d' WHERE norm='$b'";
...
global $koneksi;
	...
	$c= htmlspecialchars($_POST['c']);
	$d= htmlspecialchars($_POST['d']);
$ubah2="UPDATE `gizi` SET idlayanan='$a', norm='$b', waktulayanan='$c', diet='$d' WHERE idlayanan='$a'";
